...
&emsp;但是，在项目越来越繁杂、庞大的情况下，一个 Java 已经维护不过来了，你必须不停地加 Java 开发人手，而 Java 人员也深陷技术债中……

![图](../../../public-repertory/img/other-share-7-7.png)

&emsp;然而，就好比喜欢锻炼的不全都喜欢跑步一样，并不是所有操作数据的 Java 都喜欢写页面的，这时候就凸显了几个问题：

1. 项目越来越大，模块越来越多，需要投入的人手也越来越多，这时候需要分模块分人手地去维护。而且这个 Java 开发人员必须喜欢操作数据，也喜欢编写页面，要不然企业将面临员工频繁流动的困扰。  
2. 用户开始对界面操作提更高的要求，需要增加更多的 JavaScript 互动和特效，无形中增加 Java 开发人员的工作量。
3. 当有员工辞职后，下一名员工接手熟悉项目的时间将逐渐增加，由 1 天增长为 1 周甚至 1 个月。因为他不仅要熟悉数据的操作部分，更需要去数据页面的互动。而这段时间，辞职的员工的工作将施压到其他 Java 开发人员身上。

&emsp;综上，这时候企业不得不考虑了：能不能帮 Java 开发人员减轻负担，更好地维护发展项目。而这时候，JavaScript 经过时间的沉淀，越发凸显了它的强大，所以，在诸多因素之下，就有了前后端的分离：  
...
&emsp;上面的例子，对这两者进行了简单的比较与区分。虽然只是一个简单的说明，但是 Vue 能解决的问题远比上面的要多的多，复杂的多。  
...
&emsp;开篇点题；深入学过 Vue 开发的，那么在微信小程序、React、Angular 等框架的开发上也不会太差。  
&emsp;为什么这么说呢？因为它们都是基于 MV* 模式的一些现代前端框架。其中 Vue 与 微信小程序 的一些简单区别：  
...
* **Vue**：简单易学，灵活轻便。相对于 React 和 Angular 来说，它的学习成本低，性能好，适用于开发小型应用。缺点是在开发大型应用上比不上 React 和 Angular。
* **React**：灵活性很高，生态圈强大。相对于 Vue 来说，React 在数据操作方面更灵动；相对于 Angular 来说，它的学习成本更低。缺点是相对 Vue 比较难上手，在开发大型应用上需配合第三方库。
* **Angular**：完整的框架体系。相对于 React 和 Vue，它是一个成熟完善的框架，适用于大型项目。缺点就是体积较大、学习成本高、使用代价较大。
* **微信小程序**：简单轻便。相对于 Vue 来说，它更加便捷了，而且微信小程序有上传的代码大小限制，因为微信小程序结合微信，提供了很多 API 供开发人员使用。缺点是它依附微信平台，微信用户有多少，小程序就可能有多少用户。

&emsp;因此：

* 假如你想快速开发一个简单学习成本低的小应用，可以考虑 Vue
* 假如你想开发一个大型应用，可以考虑 Angular
* 假如你想开发一个跨平台应用，可以考虑 React

> 就 **jsliang** 本人的开发体验来说：2018 年 4 月份的毕业设计使用了 Vue 进行开发，然后相隔半年进行了微信小程序开发，发觉小程序开发比起 Vue 而言更简易了，因为它封装了很多 API 供开发人员使用。
