...
> Recently revised in **2019-09-11 11:34:39**
...
如上，我们列出了 24 种 **可能** 比较常用的 **字符串 API**，但是，如果仅仅这样列出来，那还不如直接看 MDN 关于 JavaScript 字符串的讲解：
...
4. `str.includes()`：判断一个字符串里是否包含其他字符串。
5. `str.indexOf()`：从字符串对象中返回首个被发现的给定值的索引值，如果没有找到则返回 -1。
6. `str.lastIndexOf()`：从字符串对象中返回最后一个被发现的给定值的索引值，如果没有找到则返回 -1。
7.  `str.substring()`：返回在字符串中指定两个下标之间的字符。
...
1. `str.toLocaleLowerCase()` / `str.toLowerCase()`：根据当前区域设置，将符串中的字符转换成小写。对于大多数语言来说，`toLowerCase` 的返回值是一致的。
2. `str.toLocaleUpperCase()` / `str.toUpperCase()`：根据当前区域设置，将字符串中的字符转换成大写，对于大多数语言来说，`toUpperCase` 的返回值是一致的。
...
9.  `str.substring()`：返回在字符串中指定两个下标之间的字符。
