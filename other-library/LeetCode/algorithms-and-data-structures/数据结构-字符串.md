...
> Recently revised in **2019-09-10 13:20:02**
...
| API | 功能 |
| --- | --- |
| `str.charAt(index)` X | 返回特定位置的字符。 |
| `str.charCodeAt(index)` X | 返回表示给定索引的字符的 Unicode 的值。 |
| `str.concat()` | 连接两个字符串文本，并返回一个新的字符串。 |
| `str.endsWith()` X | 判断一个字符串的是否以给定字符串结尾，结果返回布尔值。 |
| `str.includes()` X | 判断一个字符串里是否包含其他字符串。 |
| `str.indexOf()` X | 从字符串对象中返回首个被发现的给定值的索引值，如果没有找到则返回 -1。 |
| `str.lastIndexOf()` X | 从字符串对象中返回最后一个被发现的给定值的索引值，如果没有找到则返回 -1。 |
| `str.length` X | 返回了字符串的长度。 |
| `str.match()`X | 使用正则表达式与字符串相比较。 |
| `str.matchAll()`X | 返回一个包含所有匹配正则表达式及分组捕获结果的迭代器。 |
...
| `str.replace()`X | 被用来在正则表达式和字符串直接比较，然后用新的子串来替换被匹配的子串。 |
| `str.search()`X | 对正则表达式和指定字符串进行匹配搜索，返回第一个出现的匹配项的下标。 |
| `str.slice()` | 摘取一个字符串区域，返回一个新的字符串。 |
| `str.split()` | 通过分离字符串成子串，将字符串对象分割成字符串数组。 |
| `str.startsWith()` X | 判断字符串的起始位置是否匹配其他字符串中的字符。 |
| `str.substr()` X | 通过指定字符数返回在指定位置开始的字符串中的字符。 |
| `str.substring()` X | 返回在字符串中指定两个下标之间的字符。 |
