...
> Recently revised in **2019-12-09 11:36:22**
...
| <a name="catalog-chapter-five" id="catalog-chapter-five"></a>[五 初阶击鼓传花](#chapter-five) |
...
## <a name="chapter-six" id="chapter-six">六 进阶：浏览器 Event Loop</a>

> [返回目录](#chapter-one)

说到队列，如果单纯讲基础点，相信很多小伙伴都不会买单，所以咱们需要进一步探索：
...
**然后**，了解完 Event Loop 的基础内容，咱们尝试通过代码学习 Event Loop：

```js

```

**最后**，看到这里，相信小伙伴们对此有个简单了解了。

当然，咱们还未结束，为此 **jsliang** 特地翻阅了十几篇文章，从 Event Loop 的机制讲起，通过尝试描述浏览器的 Event Loop，再进一步讲解 Node.js 的 Event Loop，来帮助自己和小伙伴们深入探索，仔细了解这一块内容：
...
## <a name="chapter-seven" id="chapter-seven">六 总结</a>

> [返回目录](#chapter-one)

这样，我们就 **暂时** 完成了队列的基础了解学习，因为你站在甲板上，你是看不到整艘邮轮是怎么运行的，所以咱们慢慢探索，逐步丰富。
