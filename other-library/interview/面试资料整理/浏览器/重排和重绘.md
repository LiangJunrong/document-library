...
> Recently revised in **2020-09-21 17:03:11**
...
1. 解析 HTML，生成 `DOM` 树；解析 CSS，生成 `CSS 规则树（CSS Rule Tree）`
2. 将 `DOM Tree` 和 `CSS Rule Tree` 相结合，生成**渲染树**（`Render Tree`）
3. 渲染引擎根据 `Render Tree` 中所有节点的样式，计算它们在页面的大小和位置，最终将节点绘制在页面上。
