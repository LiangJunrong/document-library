...
Hello 大家好，我叫梁峻荣（jsliang），高考后的暑期就听大学师兄的建议，开始学习编程，那时候学习的是 C 语言，觉得世界上神奇的事情不过如此，敲两下键盘，按下回车，电脑就会响应我们。然后那时候很崇拜黑客大佬的，各种秀操作，噼里啪啦敲两下键盘，然后进度条就开始了！

大学的时候，陆陆续续学过 C、C#、.Net……等等。由于还给老师了，在这里就不多提了。

然后呢，在大二的时候，就开始接触 HTML，那时候选修了个网页设计基础，跟着老师做了个只有几个页面的静态页面。在大三的时候，参加了学校的特训班，那时候一个小组（4人），分角色（UI、前端、后端、数据库）去开发一个网站，熟悉了解前后端分离、以及项目的整体流程。

同时，由于在特训班的时候，看到后端使用 ThinkPHP（TP），觉得蛮不错的，于是自己捣鼓，使用 TP 3.2.3 + Bootstrap 3 + MySQL 打造了自己的博客（已下线）。

然后在几个月后，由于选了门 Node.js 的课，所以也跟着接触了下 Vue、Koa 这些，那时候对 npm 等诸多不懂，于是我的世界打开了个大门 —— 原来前端已经发展地这么快！

最后，我的毕业设计使用 Node.js + Vue + ElementUI + MongoDB 打造了个校园外卖、快递代拿社区单页应用。（期间跑去同学那做事，搞了个小项目，这里不多提及）

在 2018 年 5 月的时候，直接出来面试，很幸运的直接面第一家就进了（也就是我现在所处的公司），那时候表示只懂 ES5、jQuery、HTML/HTML5、CSS/CSS3 的皮毛。

就这样熟悉了三个月的业务，公司给的任务能顺利完成后，我觉得这样不行啊，外面的前端翻天覆地，我的技术还是梭哈 jQuery！于是 2018 年 8 月，jsliang 开始写 Markdown，因为 2018 年 5 月到 2018 年 8 月我是将笔记记录到了 Word 文档上的，所以我发表的第一篇文章就是这些笔记的整理，当然没人看是必须的！

然后，在 2018 年 8 月至今

1. 学 Webpack，用 Webpack 构建一个多页面配置换景，然后幸运的是，刚好碰到一个项目，于是直接用 Webpack + jQuery + VS Code 的 Live Share 进行了个公司的扒站任务，整个小组的 3 个成员直接连接我电脑进行开发！
2. 学 微信小程序，在电信直接开发微信小程序项目。
3. 学 ECharts 报表，在微信直接做了个 Vue + ECharts 展示爱音乐公司的报表数据。
4. 学（复习整理） Node，然后搭了个项目（cv.jsliang.top），并写了篇小文章（目前最高成就了，获得了 1100 多赞）。
5. 学 Vue，由于之前的 Vue 工作上有好多没有，逐渐淡忘了，所以从基础开始，准备写一套《Vue 从 0 到 1》。

以上，就是 jsliang 的编程生涯。

6. js 模块化（commonJS 与 ES6 modules 的区别）

在前端开发中，随着前后端分离，前端承受的任务越来越多，代码也就越来越长了，就好比 jsliang 个人使用 jQuery 开发的时候，懂不懂就上千行代码，这在一个编辑器上看起来就有点乱了，如果碰上没有代码折叠的编辑器（真事，不是所有前端都喜欢 VS Code 的），你就更加难受了。

所以，面向越来越多的代码，我们就急需将这些代码分门别类了，将同一类型的代码整合在一起，这时候模块化开发的理念就出来了：一个文件就是一个模块，当我们需要某个文件的时候，我们只需要引用这个模块即可……
