...
> Recently revised in **2019-10-16 01:43:24**
...
那么问题来了，如果采用 `const finalData = initData`，可不可以直接修改 `finalData` 的数据呢？它会不会影响到 `initData`？咱们可以小试一下：
...
const dataTwo = shallowCopy(dataOne); // shallowCopy 待实现
dataTwo.title = 'play';
dataTwo.number = ['null'];
console.log(dataOne);
// dataOne: { title: 'study', number: ['null'] }
console.log(dataTwo);
...
const dataFour = deepClone(dataThree); // deepClone 待实现
dataFour.title = 'play';
dataFour.number = ['null'];
console.log(dataOne);
// dataOne: { title: 'study', number: ['jsliang', 'JavaScriptLiang', 'LiangJunrong'] }
...
* 赋值：赋值会直接指向某个对象的指针。修改赋值后的数据，不管是基本数据类型还是对象数据类型，都会影响到原数据。
* 浅拷贝：浅拷贝只复制指向某个对象的指针，而不复制对象本身，新旧对象还是共享同一块内存。在浅拷贝中，修改基本数据类型不会影响原有数据的基本数据类型，修改对象数据类型会影响原有的数据类型。
* 深拷贝：深拷贝会另外创造一个一模一样的对象，新对象跟原对象不共享内存，修改新对象不会改到原对象。在深拷贝中，修改基本数据类型和对象数据类型都不会影响原有的数据类型。
