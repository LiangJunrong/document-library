...
> Recently revised in **2019-10-16 11:08:12**
...
OK，了解 **基本数据类型和引用数据类型** 后，咱们进一步探索浅拷贝和深拷贝：
...
> 这里说法可能有问题，有的大佬认为浅拷贝是拷贝一层，有的则认为浅拷贝只对基本数据类型有效，对于引用数据类型会引用其地址。
...
这边我们也可以尝试使用 `const obj2 = {...obj1}` 的形式进行浅拷贝。
...
稳健的判断 JavaScript 数据类型方式，可以符合预期的判断基本数据类型 String、Undefined 等，也可以判断 Array、Object 这些引用数据类型。
...
好的，虽然口头说着希望小伙伴们自行翻阅资料，但是 **jsliang** 也不想漏了这个问题，这里写下 **jsliang** 觉得 OK 的代码：
