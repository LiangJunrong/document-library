...
> Recently revised in **2020-10-08 22:18:24**
...
1. **客户端** 发送有一个 `SYN` 报文给 **服务器**，并且指明客户端的初始化序列号为 `ISN(c)`。此时客户端处于 `SYN_SEND` 状态。
2. **服务器** 接收到 **客户端** 的 `SYN` 报文之后，将自己的 `SYN` 报文作为应答，指明 **服务器** 的初始化系列号为 `ISN(s)`。同时会把客户端的 `ISN(c) + 1` 作为 `ACK` 的值，表示自己已经收到客户端的 `SYN`。此时 **服务器** 处于 `SYN_RECV` 状态
3. **客户端** 收到 `SYN` 报文之后，会发送一个 `ACK` 报文。当然，这个 `ACK = ISN(s) + 1`，表示收到了 **服务器** 的 `SYN` 报文。此时，**客户端** 处于 `ESTABLISHED` 状态，而 **服务器** 收到报文之后，也处于 `ESTABLISHED` 状态。双方建立了连接。
...
3. 服务器认为第二个请求是重新发的，于是向客户端发送确认报文段，同意建立连接，于是连接建立了（两次握手）
4. 这时候客户端还在等待最新的请求连接（第二次请求），自动忽略服务器发送的关于第一个请求连接的响应，也不发送数据
5. 服务端一直等待客户端发送数据，服务器资源被占用
...
* **服务器** 继续等待一会，看看客户端还有没有数据没发过来，等处理完之前的数据之后，也想断开连接了，于是也发送一个 `FIN` 报文，且指定一个序列号给客户端。此时服务器处于 `LAST_ACK` 状态。
* **客户端** 收到 `FIN` 之后，也返回一个 `ACK` 报文作为应答，且把服务器的序列号的值 + 1 作为自己的 `ACK` 报文的序列号值。此时客户端处于 `TIME_WAIT` 状态。过一阵子以确保服务器收到自己的 `ACK` 报文了，则变成 `CLOSED` 状态。服务器接受到 `ACK` 报文之后，就也处于 `CLOSED` 状态了。
...
如果有一些数据因为网络延迟等问题没有发送到，那么它直接关闭会导致这些数据没有接收到；亦或者服务端也有一些数据要发送给客户端，要确保这些数据发送完。

我们知道 `TCP` 是个可靠的棒小伙，所以它才会第一次回复客户端收到关闭连接的请求了，第二次回复客户端你发送的数据应该没延迟了，我也发送完我要发送的数据了，可以关闭了。

最后客户端接收到了，就关闭自己这边的连接，也告诉服务器它也可以关闭了。
